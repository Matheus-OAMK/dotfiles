...
ROFI_STATE_HOME="${XDG_STATE_HOME:-$HOME/.local/state}/rofi"
mkdir -p "$ROFI_STATE_HOME"
...
if [[ -f "$ROFI_STATE_HOME/fullscreen_$r_mode" ]]; then
  hypr_width="0"
  wind_border="0"
fi
...
i_override=""
i_override="configuration {icon-theme: \"${i_override:-Tela-circle-nord-dark}\";}"
rofi_args+=(
...
rofi -show "$r_mode" \
  ...
  -theme "$rofi_config" \
  -dump-theme | {
  grep -q "fullscreen.*true" && touch "$ROFI_STATE_HOME/fullscreen_$r_mode"
} || rm -f "$ROFI_STATE_HOME/fullscreen_$r_mode"
